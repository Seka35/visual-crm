-- Fix ntfy JSON parsing by adding Content-Type header
    ...
    LOOP
        ...
        IF (r.date::date + r.time::time) <= current_time_in_zone THEN
            ...
            IF target_url IS NOT NULL AND target_url != '' THEN
                ...
                -- Send POST with explicit Content-Type header
                PERFORM net.http_post(
                    url := target_url,
                    body := jsonb_build_object(
                        'topic', substring(target_url from '[^/]+$'), -- Extract topic from URL if needed, but ntfy usually takes it from URL path. 
                        -- Actually, if we post to https://ntfy.sh/topic, we don't need 'topic' in body usually, 
                        -- BUT if we post to https://ntfy.sh, we need 'topic'.
                        -- The user provides full URL like ntfy.sh/topic.
                        -- If we post to that URL, ntfy expects body to be message OR json.
                        -- To be safe with JSON body, we MUST send Content-Type: application/json.
                        'title', '📅 ' || r.title,
