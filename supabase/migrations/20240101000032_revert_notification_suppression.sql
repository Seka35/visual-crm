-- Revert process_reminders to allow double notifications (for reliability)
...
BEGIN
    -- Log start
    INSERT INTO public.debug_logs (function_name, message) VALUES ('process_reminders', 'Function started (Double Notifications Enabled)');

    -- 1. Process Calendar Events (Process ALL, even if linked to task)
    FOR r IN
        ...
        -- AND e.task_id IS NULL -- REMOVED: Allow double notifications
    LOOP
        BEGIN
